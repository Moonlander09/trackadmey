...
const UpdateAttendance = () => {
    ...
    async function fetchStudents() {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/attendance/teacher/students/${subjectId}`,
          {
            method: "GET",
            credentials: "include",
          }
        ...
        if (result.status === "success") {
          setStudents(result.session.students);

          const initialStatuses = {};
          result.session.students.forEach((s) => {
            initialStatuses[s.student._id] = s.status; // Default to absent
          });
  ...
  const markAllPresent = () => {
    const updated = {};
    students.forEach((s) => (updated[s.student._id] = "present"));
    setStatuses(updated);
  };

  const markAllAbsent = () => {
    const updated = {};
    students.forEach((s) => (updated[s.student._id] = "absent"));
    setStatuses(updated);
  };
  ...
  return (
    <div className="space-y-6 mx-2 md:m-0">
      ...
      <div className="rounded-xl bg-gradient-to-br from-white to-gray-50 border border-gray-200 shadow-lg p-6 flex flex-col items-center gap-2">
        ...
        <p className="text-[var(--text-sdy)] mb-6">Update attendance for your students</p>
        ...
        {students.length > 0 ? (
          <>
           ...
           <div className="text-center py-12 flex flex-col items-center justify-center gap-2">
                      ...
                      <p className="text-gray-500">Attendance must be recorded before any edits can be applied.</p>
                    </div>
        )}
      </div>
